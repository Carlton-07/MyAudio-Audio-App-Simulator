...
public class AudioBook extends AudioContent
{
									...
									String author, String narrator, ArrayList<String> chapterTitles, ArrayList<String> chapters)
	{
		...
		super(title, year, id, type, audioFile, length); //using super() to access parentclass members in this class
		this.author=author;
  ...
  // Print information about the audiobook. First print the basic information of the AudioContent 
	...
	{
		super.printInfo(); //overriding printinfo()
  ...
  // Play the audiobook by setting the audioFile to the current chapter title (from chapterTitles array list) 
	...
	public void play()
	{
		super.setAudioFile(chapterTitles.get(currentChapter) +".\n"+ chapters.get(currentChapter) ); //setting AudioFile to current chapter title
		super.play();// calling play() from superclass
	}
	...
	{
		for(int i=0;i<chapterTitles.size();i++){ //for loop to go through the arraylist of chapter titles and printing them out
	...
	{
		AudioBook java = (AudioBook) other; //making another object
		return super.equals(java) && this.author.equals((java.author)) && this.narrator.equals(java.narrator);
	}
